...
#define THREAD_INITIAL_TIME_SLICE ((lk_bigtime_t)50000000) // 50ms
#define THREAD_TICK_RATE ((lk_bigtime_t)10000000) // 10ms
...
void thread_resched(void)
{
    ...
    if (thread_is_real_time_or_idle(newthread)) {
        ...
        dprintf(ALWAYS, "arch_context_switch: start preempt, cpu %u, old %p (%s), new %p (%s)\n",
                cpu, oldthread, oldthread->name, newthread, newthread->name);
#endif
        timer_set_periodic(&preempt_timer[cpu], THREAD_TICK_RATE / 1000000, (timer_callback)thread_timer_tick, NULL);
    }
#endif
