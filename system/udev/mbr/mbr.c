...
#define PARTITION_TYPE_SYST 0xEA
...
static const uint8_t syst_guid[GPT_GUID_LEN] = GUID_SYSTEM_VALUE;
...
static ssize_t mbr_ioctl(mx_device_t* dev, uint32_t op, const void* cmd, size_t cmdlen, void* reply, size_t max) {
    ...
    case IOCTL_BLOCK_GET_TYPE_GUID: {
        ...
        if (device->partition.type == PARTITION_TYPE_DATA) {
            ...
            memcpy(guid, syst_guid, GPT_GUID_LEN);
            retval = GPT_GUID_LEN;
        }
        return retval;
    }
...
static void mbr_iotxn_queue(mx_device_t* dev, iotxn_t* txn) {
    ...
    // TODO(gkalsi): Check for off by ones here.
    ...
    txn->length = MIN((last - (first + off_lba) + 1) * device->blksiz, txn->length);
...
static mx_status_t mbr_open(mx_device_t* dev, mx_device_t** dev_out, uint32_t flags) {
    mbrpart_device_t* device = get_mbrpart_device(dev);
...
static int mbr_bind_thread(void* arg) {
    ...
    for (; partition_count < MBR_NUM_PARTITIONS; partition_count++) {
        ...
        xprintf("mbr: found partition, entry = %d, type = 0x%02x, start = %u, length = %u\n",
...
static mx_status_t mbr_bind(mx_driver_t* drv, mx_device_t* dev) {
    ...
    thrd_t t;
    mx_status_t rc = thrd_create_with_name(&t, mbr_bind_thread, info, "mbr-init");
    if (rc != NO_ERROR) {
        free(info);
        return rc;
    }
