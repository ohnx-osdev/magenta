...
mx_status_t usb_audio_txring_ioctl(usb_audio_txring_t* txring, uint32_t op, const void* in_buf,
    ...
    case IOCTL_AUDIO_SET_BUFFER: {
        ...
        status = mx_vmar_map(mx_vmar_root_self(), 0, handle, 0, txring->buffer_size,
    ...
    case IOCTL_AUDIO_SET_TXRING: {
        ...
        mx_status_t status = mx_vmar_map(mx_vmar_root_self(), 0, args->txring, 0,
...
static int usb_audio_txring_thread(void* arg) {
    ...
    while (1) {
        ...
        status = mx_fifo_op(txring->fifo_handle, MX_FIFO_OP_ADVANCE_TAIL, 1, &txring->fifo_state);
        if (status < 0) {
            printf("usb_audio_tx_ring MX_FIFO_OP_ADVANCE_TAIL failed: %d\n", status);
            return status;
        }
    }
...
void usb_audio_txring_stop(usb_audio_txring_t* txring) {
    ...
    mx_object_signal(txring->stop_event, 0, MX_EVENT_SIGNALED);
