...
static mx_status_t do_record(int src_fd, int dest_fd, int read_count) {
    ...
    status = mx_vmar_map(mx_vmar_root_self(), 0, buffer_vmo, 0, BUFFER_SIZE * BUFFER_COUNT,
                         MX_VM_FLAG_PERM_READ | MX_VM_FLAG_PERM_WRITE, (uintptr_t *)&buffer);
    if (status < 0) {
        printf("failed to map buffer VMO: %d\n", status);
        goto out;
    }
    status = mx_vmar_map(mx_vmar_root_self(), 0, txring_vmo, 0, sizeof(*ring) * BUFFER_COUNT,
    ...
    while (1) {
        ...
        for (int i = 0; i < empty_count; i++) {
            ...
            if (entry->status < 0) {
                ...
                if (dest_fd >= 0) {
                    int result = write(dest_fd, buffer + entry->data_offset, entry->actual);
                    if (result < 0) {
